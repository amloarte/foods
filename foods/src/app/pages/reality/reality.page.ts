...
export class RealityPage implements OnInit {
  ...
  file;
  ngOnInit() {
    // this.GetObjeto("1TgtOnx3vbXvCXe3YXYWjR8OKPjDEOOoL");
    this.idAlimento = this.route.snapshot.params['id'];
    this.getAlimentos();
    // var that = this;
    // setTimeout(function () {
    //   that.GetObjeto(this.modeloObjeto3d);
    // }, 2000);
    // console.log('NGONINITID--------->>>>>>>>>.', this.modeloObjeto3d)
  }
  ...
  getAlimentos() {
    this.alimentoService.getOneAlimento(this.idAlimento)
      .subscribe(res => {
        ...
        // this.modeloObjeto3d = this.oneAlimento.modelo3d;
        this.GetObjeto(this.oneAlimento.modelo3d)
      })
  }
  ...
  GetObjeto(idObjeto) {
    console.log('NGONINITID--------->>>>>>>>>.', this.modeloObjeto3d)
    this.googleS.get_objectfromDriven(idObjeto).subscribe(res => {
